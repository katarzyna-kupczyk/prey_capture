...
def divide_and_preprocess_3_params(data, needed_params, final_dict):
    for p0 in data[needed_params[0]].unique():
        for p1 in data[needed_params[1]].unique():
            for p2 in data[needed_params[2]].unique():
                df = data[(data[needed_params[0]] == p0)
                        & (data[needed_params[1]] == p1)
                        & (data[needed_params[2]] == p2)].reset_index().drop(
                            columns=['index'])
